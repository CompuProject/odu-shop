    ...
    public static function init ($orderID, &$eventName, &$arFields)
    {
        if(CModule::IncludeModule("sale") && CModule::IncludeModule("iblock"))
        {
            ...
            while ($arProps = $dbBasketItems->Fetch())
            {
                ...
                $arrayRes[$counter]["BRAND"] = CP_CatalogHelper::getEnumPropertyCodeFromId(CP_CatalogHelper::getPropertyValue($arProps["PRODUCT_ID"],"BRAND"));
            ...
            $strOrderList .= '<th>Бренд/Наименование</th>';
            $strOrderList .= '<th>Количество</th>';
            $strOrderList .= '<th>Цена</th>';
            $strOrderList .= '<th>Сумма</th>';
            $strOrderList .= '</tr>';
            foreach ($arrayRes as $item) {
                $strOrderList .= '<tr><td>'.$item["PRODUCT_CODE"].'</td><td>'.$item["BRAND"].' - '.$item["NAME"].'</td><td>'.$item["QUANTITY"].'</td><td>'.substr($item["PRICE"],0,-2).'</td><td>'.(float)$item["SUMM"].'</td></tr>';
            }
            $strOrderList .= "</table>";
